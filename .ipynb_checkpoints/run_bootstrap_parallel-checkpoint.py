...
def run_batch(num_estimates, N_max, n_total, alpha):
...
def check_independent_last(seed,alpha,s_0,N_traj,n0,N_iters,num_workers=1):
    N_max = 2048
    np.random.seed(seed)
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as pool:
        ...
        for i in range(n_loops):
            futures.append(pool.submit(run_batch, num_estimates, N_max, n_total, alpha))
...
pr_iter = check_independent_last(seed + i,alpha,s0,N_traj,N_b,N_iters[i], num_workers=16)
